...
df<-as_tibble(t(time_end))
df%>%filter(bankroll==0)%>%summarise(mean_t=mean(time))
```

    d. the mean and variance of your bankroll after 100 hands (including busts).
    
```{r}
df%>%filter(time>100)%>%summarise(mean_b=mean(bankroll),var_b=var(bankroll))
```

    e. the mean and variance of your bankroll after 500 hands (including busts).
  
```{r}
df%>%filter(time>500)%>%summarise(mean_b=mean(bankroll),var_b=var(bankroll))
```
